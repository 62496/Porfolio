...
const AddBooksToSeriesModal = ({
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Manage Books in Series">
            <p className="text-sm text-[#6e6e73] -mt-2 mb-4">{series.title}</p>

            {/* Current books in series */}
            {seriesBooks.length > 0 && (
                <div className="mb-6">
                    <h3 className="text-sm font-medium text-[#1d1d1f] mb-3">
                        Books in this series ({seriesBooks.length})
                    </h3>
            ...
            <div className="mt-4 space-y-2 max-h-[300px] overflow-y-auto">
                {filteredBooks.length === 0 ? (
                    ...
                    filteredBooks.slice(0, 20).map((book) => {
                        const inSeries = isBookInSeries(book.isbn);
                        return (
                            <div
                                key={book.isbn}
...
export default AddBooksToSeriesModal;
